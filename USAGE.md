...
>
> **Documentation style:** `writer-agent` dùng **một template duy nhất** — [templates/document-style.md](./templates/document-style.md) (embedded CSS, diagram panels, section headers, item cards, Mermaid patterns). Repo đích có convention mạnh hơn (`docs/DOCUMENT-STYLE.md`, `docs/REPORT-STYLE.md`) thì ưu tiên local.
...
Style: follow cursor-agents `templates/document-style.md` unless the target repo has stronger local doc conventions.
```
